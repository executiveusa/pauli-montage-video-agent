...
The local projection additionally requires the recorded merge to be an ancestor of fetched `origin/main`, its tree to match the canonical remote PR head tree, its first parent to equal the rollback baseline, and the Slice's exact OpenSpec proposal to appear for the first time in that merge. `postMerge.mainSha` is proved by the canonical remote-main checks. Each `treeChecks` URL must identify a successful retry of the trusted PR-head workflow whose `run_started_at` is later than GitHub's canonical `merged_at` timestamp. The retry checks the byte-identical squash tree only after the merge exists on `main`, and its full-history checkout fetches the now-canonical `origin/main`, so merge-dependent validation executes on post-merge repository state; the original pre-merge run is rejected. PR numbers, PR heads, and merge SHAs are globally unique across Slice evidence.
