...
5. if Windows `venv/ensurepip` fails, automatically removes the incomplete environment and falls back to an isolated `E:\MONTAGE_RUNTIME\python-packages` directory using the working system Python;
...
Rerun the latest setup script after `git pull origin main`. The installer now detects an incomplete `.venv`, removes only that Montage runtime folder, and retries using the fallback package directory on E:. Do not manually delete source footage.
