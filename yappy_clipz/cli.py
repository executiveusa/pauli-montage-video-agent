...
def _json_file(value: Path) -> dict:
    try:
        ...
        raise ServiceValidationError(f"JSON file is unreadable: {exc}") from exc
    if not isinstance(payload, dict):
        raise ServiceValidationError("JSON file must contain an object")
    return payload


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="yappy-clipz")
    parser.add_argument("--token", help="Hosted bearer token; defaults to YAPPY_ACCESS_TOKEN")
    ...
    create.add_argument("--tenant"); create.add_argument("--slug", required=True); create.add_argument("--title", required=True)
    ...
    listing = project_actions.add_parser("list"); listing.add_argument("--tenant")
    get = project_actions.add_parser("get"); get.add_argument("--tenant"); get.add_argument("project_id")
    validate = project_actions.add_parser("validate"); validate.add_argument("--tenant"); validate.add_argument("project_id")
    ...
    timeline_get = timeline_actions.add_parser("get"); timeline_get.add_argument("--tenant"); timeline_get.add_argument("project_id")
    timeline_replace = timeline_actions.add_parser("replace"); timeline_replace.add_argument("--tenant")
...
def _action_context(args: argparse.Namespace, runtime: ApplicationRuntime) -> ActionContext:
    token = getattr(args, "token", None) or os.environ.get("YAPPY_ACCESS_TOKEN")
    if token:
        authorization = token if token.startswith("Bearer ") else "Bearer " + token
        try:
            principal = runtime.auth.verify_bearer(authorization)
    ...
    if runtime.settings.auth_mode == "hosted":
        raise ActionProblem("authentication_required", "--token or YAPPY_ACCESS_TOKEN is required in hosted mode", 401)
    tenant = getattr(args, "tenant", None)
    return ActionContext(
        tenant_id=tenant,
        actor_id="cli",
        scopes=tuple(runtime.auth.DEFAULT_SCOPES),
        approved=getattr(args, "approved", False),
        idempotency_key=getattr(args, "idempotency_key", None),
        correlation_id=getattr(args, "correlation_id", None),
        causation_id=getattr(args, "causation_id", None),
...
def execute(args: argparse.Namespace, service: StudioService, runtime: ApplicationRuntime | None = None) -> object:
    ...
    context = _action_context(args, runtime)
    if args.resource == "action" and args.action == "run":
        return runtime.dispatcher.dispatch(args.action_id, _json_file(args.input), context=context)
    if args.resource == "project":
        if args.action == "create":
            payload = {"slug":args.slug,"title":args.title,"objective":args.objective,"deliverables":args.deliverable,"audience":args.audience,"constraints":args.constraint,"qualityLane":args.quality_lane}
            return runtime.dispatcher.dispatch("project.create", payload, context=context)["result"]
        if args.action == "list":
            return runtime.dispatcher.dispatch("project.list", context=context)["result"]
        if args.action == "get":
            return runtime.dispatcher.dispatch("project.get", {"projectId":args.project_id}, context=context)["result"]
        if args.action == "validate":
            return runtime.dispatcher.dispatch("project.validate", {"projectId":args.project_id}, context=context)["result"]
    if args.resource == "timeline":
        if args.action == "get":
            return runtime.dispatcher.dispatch("timeline.get", {"projectId":args.project_id}, context=context)["result"]
        if args.action == "replace":
            return runtime.dispatcher.dispatch("timeline.replace", {"projectId":args.project_id,"expectedVersion":args.expected_version,"timeline":_json_file(args.file)}, context=context)["result"]
...
def main(argv: Sequence[str] | None = None, service: StudioService | None = None, runtime: ApplicationRuntime | None = None) -> int:
    ...
    try:
        result = execute(args, active_service, active_runtime)
