...
class OmniRouter:
 ...
 def estimate_model(model:dict[str,Any],payload:dict[str,Any])->dict[str,Any]:
  pricing=model.get("pricing") or {};currency=pricing.get("currency","USD");unit=pricing.get("unit");amount=None
  if unit=="second":
   duration=payload.get("duration",payload.get("durationSeconds",8));duration=8 if duration=="auto" else float(duration);resolution=payload.get("resolution",model.get("defaults",{}).get("resolution","720p"));rate=(pricing.get("ratesPerSecondUsd") or {}).get(resolution);amount=duration*float(rate) if rate is not None else None
 ...
 def plan(self,*,capability:str,payload:dict[str,Any],quality_lane:str="economy",max_cost:float|None=None,preferred_provider:str|None=None,privacy_lane:str="cloud",allow_experimental:bool=True)->dict[str,Any]:
  ...
  for summary in self.catalog.list():
   ...
   for model in provider.get("models",[]):
    ...
    if max_cost is not None and amount is not None and amount>max_cost:reasons.append("over_cost_ceiling");score-=1000;eligible=False
