...
from .operations import JsonOperationStore,OperationsService,PostgresOperationStore
...
class ApplicationRuntime:
 settings:Settings;service:StudioService;capabilities:CapabilityRegistry;prompt_locker:PromptLocker;provider_catalog:ProviderCatalog;icm:IcmRuntime;fal:FalProviderAdapter;auth:AuthService;storage:ObjectStorage;assets:AssetService;operations:OperationsService;router:OmniRouter;dispatcher:HostedActionDispatcher

def create_repository(settings:Settings)->ProjectRepository:
 return PostgresProjectRepository(settings.database_url or "") if settings.repository_backend=="postgres" else FileProjectRepository(settings.project_root)
def create_storage(settings:Settings)->ObjectStorage:
 return S3ObjectStorage(bucket=settings.storage_bucket or "",region=settings.storage_region,endpoint_url=settings.storage_endpoint_url) if settings.storage_backend=="s3" else LocalObjectStorage(settings.resolved_storage_root)
...
def create_runtime(settings:Settings|None=None,*,service:StudioService|None=None,http_client:Any|None=None)->ApplicationRuntime:
 ...
 operation_store=PostgresOperationStore(resolved.database_url) if resolved.repository_backend=="postgres" and resolved.database_url else JsonOperationStore(resolved.project_root.parent/"operations.json")
 operations=OperationsService(operation_store);router=OmniRouter(provider_catalog)
 dispatcher=HostedActionDispatcher(service=active_service,registry=capabilities,prompt_locker=prompt_locker,provider_catalog=provider_catalog,fal=fal,icm=icm,auth=auth,assets=assets,operations=operations,router=router)
