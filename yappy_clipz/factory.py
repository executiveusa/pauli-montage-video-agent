...
def create_repository(settings: Settings) -> ProjectRepository:
    if settings.repository_backend == "postgres": return PostgresProjectRepository(settings.database_url or "")
    return FileProjectRepository(settings.project_root)


def create_storage(settings: Settings) -> ObjectStorage:
    if settings.storage_backend == "s3":
        return S3ObjectStorage(bucket=settings.storage_bucket or "", region=settings.storage_region, endpoint_url=settings.storage_endpoint_url)
    return LocalObjectStorage(settings.resolved_storage_root)
...
def create_runtime(settings: Settings | None = None, *, service: StudioService | None = None, http_client: Any | None = None) -> ApplicationRuntime:
    resolved = settings or Settings.from_env(); active_service = service or create_service(resolved)
    capabilities = HostedCapabilityRegistry(default_registry())
    prompt_locker = PromptLocker(resolved.resolved_prompt_root); provider_catalog = ProviderCatalog(resolved.resolved_provider_root); icm = IcmRuntime(resolved.resolved_icm_runtime_root)
    revocations = PostgresRevocationStore(resolved.database_url) if resolved.repository_backend == "postgres" and resolved.database_url else MemoryRevocationStore()
    auth = AuthService(mode=resolved.auth_mode, signing_secret_env=resolved.auth_signing_secret_env, owner_username=resolved.auth_owner_username, owner_password_env=resolved.auth_owner_password_env, owner_tenant_id=resolved.auth_owner_tenant_id, session_ttl_seconds=resolved.auth_session_ttl_seconds, service_ttl_seconds=resolved.auth_service_ttl_seconds, revocations=revocations)
    fal = FalProviderAdapter(provider_catalog, FalSettings(queue_base_url=resolved.fal_queue_base_url,key_env=resolved.fal_key_env,execution_enabled=resolved.fal_execution_enabled,store_io=resolved.fal_store_io,timeout_seconds=resolved.fal_timeout_seconds),http_client=http_client)
    ...
    dispatcher = HostedActionDispatcher(service=active_service,registry=capabilities,prompt_locker=prompt_locker,provider_catalog=provider_catalog,fal=fal,icm=icm,auth=auth,assets=assets)
    return ApplicationRuntime(settings=resolved,service=active_service,capabilities=capabilities,prompt_locker=prompt_locker,provider_catalog=provider_catalog,fal=fal,icm=icm,auth=auth,storage=storage,assets=assets,dispatcher=dispatcher)
