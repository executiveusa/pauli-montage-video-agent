...
class ApplicationRuntime:
 settings:Settings;service:StudioService;capabilities:CapabilityRegistry;prompt_locker:PromptLocker;provider_catalog:ProviderCatalog;icm:IcmRuntime;fal:ExtendedFalProviderAdapter;auth:AuthService;storage:ObjectStorage;assets:AssetService;operations:BudgetedOperationsService;router:OmniRouter;generation:GenerationService;dispatcher:GenerationActionDispatcher
...
def create_runtime(settings:Settings|None=None,*,service:StudioService|None=None,http_client:Any|None=None)->ApplicationRuntime:
 resolved=settings or Settings.from_env();active_service=service or create_service(resolved);capabilities=GenerationCapabilityRegistry(OperationsCapabilityRegistry(HostedCapabilityRegistry(default_registry())));prompt_locker=PromptLocker(resolved.resolved_prompt_root);provider_catalog=ProviderCatalog(resolved.resolved_provider_root);icm=IcmRuntime(resolved.resolved_icm_runtime_root)
 ...
 dispatcher=GenerationActionDispatcher(service=active_service,registry=capabilities,prompt_locker=prompt_locker,provider_catalog=provider_catalog,fal=fal,icm=icm,auth=auth,assets=assets,operations=operations,router=router,generation=generation)
 return ApplicationRuntime(settings=resolved,service=active_service,capabilities=capabilities,prompt_locker=prompt_locker,provider_catalog=provider_catalog,fal=fal,icm=icm,auth=auth,storage=storage,assets=assets,operations=operations,router=router,generation=generation,dispatcher=dispatcher)
