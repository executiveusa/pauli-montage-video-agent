...
def _http_error(exc: Exception) -> HTTPException:
    if isinstance(exc, (AuthenticationRequired, AuthConfigurationError)):
        status = 401 if isinstance(exc, AuthenticationRequired) else 503
        return HTTPException(status_code=status, detail=str(exc))
    if isinstance(exc, AuthorizationDenied):
        return HTTPException(status_code=403, detail=str(exc))
    if isinstance(exc, ProjectNotFound):
        return HTTPException(status_code=404, detail="project not found")
    if isinstance(exc, (AssetNotFound, ObjectNotFound)):
        return HTTPException(status_code=404, detail="resource not found")
    if isinstance(exc, TransferInvalid):
        return HTTPException(status_code=403, detail=str(exc))
    ...
    if isinstance(
        exc,
        (ServiceValidationError, RepositoryError, AssetError, StorageError, ValueError),
    ):
...
def create_app(
    ...
    app = FastAPI(title="YAPPY-CLIPZ Studio API", version="1.2.1")
    ...
    def principal(request: Request, tenant_header: str | None = None) -> Principal:
        if active_runtime.settings.auth_mode == "local":
            ...
            return Principal(
                tenant_header,
                "local-api",
                tuple(active_runtime.auth.DEFAULT_SCOPES),
                "local",
                "local",
                0,
                2**31,
    ...
    def require(resolved: Principal, *scopes: str) -> Principal:
        ...
        if not resolved.allows(required):
            raise AuthorizationDenied(
                "caller lacks required scopes: " + ", ".join(sorted(required))
            )
        return resolved
    ...
    def health() -> dict[str, Any]:
        return {
            "ok": True,
            "service": "yappy-clipz-studio-api",
            "apiVersion": "v1",
            "repository": active_runtime.settings.repository_backend,
            "authMode": active_runtime.settings.auth_mode,
            "authConfigured": active_runtime.auth.configured,
            "storage": active_runtime.storage.storage_type,
        }
    ...
    def inspect_session(
        request: Request,
        tenant: OptionalTenantHeader = None,
    ) -> dict[str, Any]:
        try:
            return active_runtime.dispatcher.dispatch(
                "session.inspect",
                context=context_for(request, tenant),
            )["result"]
    ...
    def create_token(
        ...
        tenant: OptionalTenantHeader = None,
        idempotency: OptionalIdempotencyHeader = None,
    ) -> dict[str, Any]:
        try:
            return active_runtime.dispatcher.dispatch(
                "token.create",
    ...
    def revoke_token(
        ...
        try:
            return active_runtime.dispatcher.dispatch(
                "token.revoke",
                {"token": payload.token},
                context=context_for(request, tenant, approved=payload.approved),
            )["result"]
        except ActionProblem as exc:
            raise HTTPException(status_code=exc.status, detail=exc.message) from exc
        except Exception as exc:
    ...
    def run_action(
        ...
        tenant: OptionalTenantHeader = None,
        idempotency_header: OptionalIdempotencyHeader = None,
        correlation_header: OptionalCorrelationHeader = None,
    ) -> dict[str, Any] | JSONResponse:
        try:
            protected = not action_id.startswith("capabilities.") and not action_id.startswith(
                "system."
            )
            if protected:
                context = context_for(
    ...
    async def upload_transfer(
        ...
        try:
            resolved = require(principal(request, tenant), "asset:write")
            length = request.headers.get("content-length")
            if length is None or int(length) > active_runtime.settings.max_upload_bytes:
                raise AssetError("valid bounded Content-Length is required")
            data = await request.body()
            return active_runtime.assets.accept_upload(
                tenant_id=resolved.tenant_id,
                token=token,
                data=data,
    ...
    def download_transfer(
        ...
        try:
            resolved = require(principal(request, tenant), "asset:read")
            data, mime = active_runtime.assets.download(
                tenant_id=resolved.tenant_id,
                token=token,
            )
            return Response(
    ...
    def create_project(
        ...
        try:
            resolved = require(principal(request, tenant), "project:write")
    ...
    def list_projects(
        ...
        try:
            resolved = require(principal(request, tenant), "project:read")
            return active.list_projects(tenant_id=resolved.tenant_id)
        except Exception as exc:
            raise _http_error(exc) from exc

    # Specific project subresources are registered before the catch-all project
    # route. The `path` converter preserves every contract-valid opaque ID,
    # including IDs that contain encoded slash characters.
    @app.post("/api/v1/projects/{project_id:path}/validate")
    def validate_stored_project(
        project_id: str,
        request: Request,
        tenant: OptionalTenantHeader = None,
    ) -> dict[str, Any]:
        try:
            resolved = require(principal(request, tenant), "project:read")
            return active.validate_project(
                tenant_id=resolved.tenant_id,
    ...
    def get_timeline(
        ...
        try:
            resolved = require(
                principal(request, tenant),
                "project:read",
                "timeline:read",
            )
            return active.get_timeline(
                tenant_id=resolved.tenant_id,
                project_id=project_id,
            )
        except Exception as exc:
    ...
    def replace_timeline(
        project_id: str,
        request: Request,
        payload: ReplaceTimelineRequest,
        tenant: OptionalTenantHeader = None,
    ) -> dict[str, Any]:
        try:
            resolved = require(
