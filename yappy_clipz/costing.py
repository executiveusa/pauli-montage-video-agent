...
class BudgetedOperationsService(OperationsService):
 def effective_spend(self,tenant_id:str,project_id:str)->float:
  by_job={}
  for entry in self.store.list_costs(tenant_id,project_id):by_job.setdefault(entry["jobId"],[]).append(entry)
  total=0.0
  for entries in by_job.values():
   actual=[e for e in entries if e.get("kind")=="actual" and e.get("status")!="reversed"]
   released=any(e.get("kind")=="release" for e in entries)
   if actual:total+=sum(float(e["amount"]) for e in actual)
   elif not released:total+=sum(float(e["amount"]) for e in entries if e.get("kind")=="reserved" and e.get("status")=="active")
  return total
 ...
 def release_cost(self,*,tenant_id,project_id,job_id,reason):
  entry={"id":f"cost_{uuid4().hex[:24]}","tenantId":tenant_id,"projectId":project_id,"jobId":job_id,"kind":"release","status":"released","amount":0.0,"currency":"USD","reason":reason,"createdAt":now_iso()}
  self.store.append_cost(entry);self.event(tenant_id,project_id,"cost.released",entry["id"],{"jobId":job_id,"reason":reason});return entry
